                                    ...
                                    filteredMembers.map((member) => (
                                        <div key={member.id} className="flex items-center gap-3 px-4 py-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="font-medium text-sm text-gray-900 truncate">{member.name || 'Unknown'}</div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    ...
                                                    {isWorkspaceOwner && (
                                                        <Badge variant={getStatusBadgeColor(member.status || 'inactive') as any} dot>
                                                            {member.status === 'active' ? 'Active' : 'Inactive'}
                                                        </Badge>
                                                    )}
                                                </div>
                                            ...
                                            {openDropdownId === member.id && dropdownPosition && (
                                                <>
                                                    ...
                                                    >
                                                        {canChangeRoles && (
                                                            <button onClick={() => handleChangeRole(member)} className="w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2">
                                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                                                                Change Role
                                                            </button>
                                                        )}
                                                        {canRemoveMembers && (
                                                            <button onClick={() => handleRemoveMember(member)} className="w-full px-4 py-2 text-left text-sm text-red-600 hover:bg-red-50 flex items-center gap-2">
