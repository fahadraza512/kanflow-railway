                ...
                <>
                    ...
                    <div className="sm:hidden space-y-2">
                        {teamWorkload.map((member) => {
                            const rate = member.taskCount > 0 ? Math.round((member.completedCount / member.taskCount) * 100) : 0;
                            return (
                                <div key={member.userId} className="bg-gray-50 rounded-lg p-3">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-xs font-semibold text-gray-900">{member.name}</span>
                                        <span className="text-xs font-bold text-blue-600">{rate}%</span>
                                    </div>
                                    <div className="w-full bg-gray-200 rounded-full h-1.5 overflow-hidden mb-2">
                                        <div className="h-full bg-blue-600 rounded-full" style={{ width: `${rate}%` }} />
                                    </div>
                                    <div className="flex gap-3 text-[10px] text-gray-500">
                                        <span>Total: <span className="font-bold text-gray-900">{member.taskCount}</span></span>
                                        <span>Done: <span className="font-bold text-green-600">{member.completedCount}</span></span>
                                    </div>
                                </div>
                            );
                        })}
